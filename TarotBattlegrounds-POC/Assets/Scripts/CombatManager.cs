...
public static class CombatManager
{
    public static (int damage, string winner) SimulateBattle(List<Card> pBoard, List<Card> aBoard, int tavernTier, string pName, string aName)
    {
        ...
        while (hasValidAttack)
        {
            ...
            if (attacker != null)
            {
                ...
                if (target != null)
                {
                    ...
                    // Apply counterattack damage (even if target dies)
        ...
        Debug.Log($"Outcome: {(winner == "Tie" ? "Both boards empty, Tie" : $"{winner} wins")}, Surviving tier sum = {survivingTier}, Damage = {damage}");
        return (damage, winner);
    }
}