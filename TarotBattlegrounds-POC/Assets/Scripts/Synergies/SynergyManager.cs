...
public class SynergyManager : MonoBehaviour
{
    ...
    /// Data construction moved to DefaultSynergyFactory (spec §4) — this method still owns which
    /// tribes are missing and only builds+caches those, preserving the original behavior verbatim.
    /// </summary>
    private void EnsureDefaultSynergies()
    {
        TribeType[] defaultableTribes =
        {
            TribeType.Stars, TribeType.Coins, TribeType.Pentacles,
            TribeType.Cups, TribeType.Swords, TribeType.Wands
        };

        foreach (TribeType tribe in defaultableTribes)
        {
            if (_synergyByTribe.ContainsKey(tribe)) continue;

            TribeSynergy synergy = DefaultSynergyFactory.CreateDefault(tribe);
            if (synergy != null)
                _synergyByTribe[tribe] = synergy;
        }
    }
